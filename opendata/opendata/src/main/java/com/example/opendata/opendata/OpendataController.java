package com.example.opendata.opendata;
...
public class OpendataController {
	...
	public String opendata() throws IOException {
     ...
     urlBuilder.append("&" + URLEncoder.encode("inqryDiv","UTF-8") + "=" + URLEncoder.encode("1", "UTF-8")); /*검색하고자하는 조회구분 1:공고게시일시, 2:개찰일시 (방위사업청 연계건의 경우 조회구분) 1. 공고게시일시 : 공고일자(pblancDate)*/
     urlBuilder.append("&" + URLEncoder.encode("inqryBgnDt","UTF-8") + "=" + URLEncoder.encode("202408020000", "UTF-8")); /*검색하고자 하는 조회시작일시*/
     urlBuilder.append("&" + URLEncoder.encode("inqryEndDt","UTF-8") + "=" + URLEncoder.encode("202408022359", "UTF-8")); /*검색하고자 하는 조회종료일시*/
     urlBuilder.append("&" + URLEncoder.encode("type","UTF-8") + "=" + URLEncoder.encode("json", "UTF-8")); /*오픈API 리턴 타입을 JSON으로 받고 싶을 경우 'json' 으로 지정*/
     urlBuilder.append("&" + URLEncoder.encode("bidClseExcpYn","UTF-8") + "=" + URLEncoder.encode("Y", "UTF-8")); /*입찰마감건을 제외하고 검색하고자 하는 경우 Y*/
     ...
     List<Map<String,String>> resultMap = objectMapper.readValue(sb.toString(), List.class);
